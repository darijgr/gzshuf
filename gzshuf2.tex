...
only if the four Conditions \textbf{(i')}, \textbf{(ii')}, \textbf{(iii')} and
\textbf{(iv')} in Definition \ref{def.amen} hold. Consider these four
conditions. Since Condition \textbf{(iv')} automatically holds (since we
assumed $g$ to be weakly increasing), we thus only need to discuss the other three:

\begin{itemize}
\item Condition \textbf{(i')} is equivalent to the statement that every
...
\item Condition \textbf{(ii')} is equivalent to the statement that every peak
...
\item Condition \textbf{(iii')} is equivalent to the statement that every
...
Combining all of these insights, we conclude that the four Conditions
...
Conditions \textbf{(i')}, \textbf{(ii')}, \textbf{(iii')} and \textbf{(iv')}
...
Condition \textbf{(iv')} in Definition \ref{def.amen}). Hence, Proposition
...
In other words, a set of integers is lacunar if and only if it contains
no two consecutive integers.
For example, the set $\left\{2, 5, 7\right\}$ is lacunar, while the
set $\left\{2, 5, 6\right\}$ is not.

Lacunar sets of integers are also called \textit{sparse} sets in some
of the literature (though the latter word has several competing meanings).
...
$\operatorname*{st}$-equivalence class of the empty permutation.
\begin{verlong}
(In the
