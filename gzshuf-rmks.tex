...
%TCIDATA{LastRevised=Tuesday, August 22, 2017 09:40:11}
...
I am going to define \textit{$\mathcal{Z}$-enriched }$P$\textit{-partitions}, which are a
straightforward generalization of the notions of \textquotedblleft%
...
\begin{definition}
\label{def.ambivPp}Now, let $\left(  P,\gamma\right)  $ be a labeled poset. An
\textit{$\mathcal{Z}$-enriched }$\left(  P,\gamma\right)  $\textit{-partition} means a map
$f:P\rightarrow\mathcal{Z}$ such that for all $x<y$ in $P$, the following
conditions hold:
...
$\mathcal{Z}$-enriched $\left(  P,\gamma\right)  $-partitions are the enriched $\left(
...
+,-\right\}  \right)  \setminus\left\{  -0\right\}  $, then the $\mathcal{Z}$-enriched
$\left(  P,\gamma\right)  $-partitions are the left enriched $\left(
P,\gamma\right)  $-partitions. Note that $+0$ and $-0$ here stand for the
pairs $\left(  0,+\right)  $ and $\left(  0,-\right)  $; thus, they are not
the same thing.
\end{example}
...
Let us prove some basic facts about $\mathcal{Z}$-enriched $\left(  P,\gamma\right)
$-partitions, straightforwardly generalizing classical results proven by
Stanley and Gessel (for the case of \textquotedblleft plain\textquotedblright%
\ $\left(  P,\gamma\right)  $-partitions), Stembridge (for enriched $\left(
P,\gamma\right)  $-partitions) and Petersen (for left enriched $\left(
P,\gamma\right)  $-partitions):
...
definition of an $\mathcal{Z}$-enriched $\left(  \left[  n\right]  ,\pi\right)
$-partition. Let me spell it out: Let $f\in\mathcal{E}\left(  \left[
n\right]  ,\pi\right)  $. Thus, $f$ is an $\mathcal{Z}$-enriched $\left(  \left[
n\right]  ,\pi\right)  $-partition. In other words, $f$ is a map $\left[
n\right]  \rightarrow\mathcal{Z}$ such that for all $x<y$ in $\left[
...
\begin{proof}
[Proof of Proposition \ref{prop.Epk-formula2}.]This follow from Proposition
\ref{prop.Epk-formula} once we know that the $\pi$-amenable maps $g:\left[
...
\pi\subseteq\operatorname*{FE}\left(  g\right)  $. But this is easy to check.
(The main idea: If $g:\left[  n\right]  \rightarrow\mathcal{N}$ is a weakly
increasing map, then all nonempty fibers $g^{-1}\left(  h\right)  $ of $g$ are
intervals, and Conditions \textbf{(i')}, \textbf{(ii')} and \textbf{(iii')} in
the definition of \textquotedblleft$\pi$-amenable\textquotedblright\ say
precisely that no peak of $\pi$ can appear in the interior of any such fiber;
i.e., all peaks must appear at fiber-ends. Of course, this needs some
exceptions for $g^{-1}\left(  0\right)  $ and $g^{-1}\left(  \infty\right)  $,
but this is all straightforward.)
\end{proof}
...
\begin{proof}
[Proof of Proposition \ref{prop.when-Epk}.]Left to the reader.
\end{proof}
...
where $J$ and $K$ are two compositions of the same size satisfying
$\operatorname*{st}J=\operatorname*{st}K$. We shall refer to $\mathcal{K}%
...
we fix an element $L_{\alpha}$ of $\alpha$. Then, each $\operatorname*{st}%
$-equivalence class $\alpha$ of compositions and each composition $L\in\alpha$
satisfy $\operatorname*{st}L=\operatorname*{st}\left(  L_{\alpha}\right)  $
(because both compositions $L$ and $L_{\alpha}$ lie in the same
$\operatorname*{st}$-equivalence class $\alpha$) and therefore
\begin{equation}
...
$\operatorname*{QSym}$ spanned by all elements of the form $F_{J}-F_{K}$,
where $J$ and $K$ are two compositions of the same size satisfying
$\operatorname*{st}J=\operatorname*{st}K$. Hence, in order to prove that
$\mathcal{K}_{\operatorname*{st}}\subseteq\operatorname*{Ker}\left(
\phi_{\operatorname*{st}}\right)  $, it suffices to show that $F_{J}-F_{K}%
\in\operatorname*{Ker}\left(  \phi_{\operatorname*{st}}\right)  $, whenever
$J$ and $K$ are two compositions of the same size satisfying
$\operatorname*{st}J=\operatorname*{st}K$. So let $J$ and $K$ be two
compositions of the same size satisfying $\operatorname*{st}%
J=\operatorname*{st}K$. We must show that $F_{J}-F_{K}\in\operatorname*{Ker}%
\left(  \phi_{\operatorname*{st}}\right)  $.
\par
The two compositions $J$ and $K$ have the same size and satisfy
$\operatorname*{st}J=\operatorname*{st}K$. In other words, the two
compositions $J$ and $K$ are $\operatorname*{st}$-equivalence. Hence, they lie
in one and the same $\operatorname*{st}$-equivalence class. Let $\alpha$ be
this $\operatorname*{st}$-equivalence class. Then, $J\in\alpha$ and therefore
$\phi_{\operatorname*{st}}\left(  F_{J}\right)  =u_{\alpha}$ (by
...
}\left(  \operatorname*{QSym}\right)  \cong\operatorname*{QSym}/\mathcal{K}%
_{\operatorname*{st}}$. But the second sentence of \cite[Theorem 4.3]{part1}
shows that $\mathcal{A}_{\operatorname*{st}}\cong A$. Thus, $\mathcal{A}%
_{\operatorname*{st}}\cong A\cong\operatorname*{QSym}/\mathcal{K}%
...
elements of the form $F_{J}-F_{K}$, where $J$ and $K$ are two compositions of
the same size satisfying $\operatorname*{Epk}J=\operatorname*{Epk}K$. Thus, it
suffices to show that if $J$ and $K$ are two compositions of the same size
satisfying $\operatorname*{Epk}J=\operatorname*{Epk}K$, then $F_{J}-F_{K}%
\in\mathcal{K}^{\prime}$.

So let $J$ and $K$ be two compositions of the same size satisfying
$\operatorname*{Epk}J=\operatorname*{Epk}K$. We must prove that $F_{J}%
-F_{K}\in\mathcal{K}^{\prime}$.

Let $n=\left\vert J\right\vert =\left\vert K\right\vert $. (This is
well-defined, since $J$ and $K$ are compositions of the same size.)
...
composition $K\odot G$ is obtained by \textquotedblleft
splitting\textquotedblright\ the entry $j_{\ell}>2$ into two consecutive
entries $1$ and $j_{\ell}$, and that this entry $j_{\ell}$ was not the first
