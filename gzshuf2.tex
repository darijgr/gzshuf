...
%TCIDATA{LastRevised=Thursday, October 05, 2017 22:42:17}
...
\begin{definition}
\label{def.perm}Let $n\in\mathbb{Z}$. We shall use the notation $\left[
n\right]  $ for the totally ordered set $\left\{  1,2,\ldots,n\right\}  $
(with the usual order relation inherited from $\mathbb{Z}$). Note that
$\left[  n\right]  =\varnothing$ when $n\leq0$.

An $n$\textit{-permutation} shall mean a word with $n$ letters, which are
distinct and belong to $\left\{  1,2,3,\ldots\right\}  $. Equivalently, an
$n$-permutation shall be regarded as an injective map $\left[  n\right]
\rightarrow\left\{  1,2,3,\ldots\right\}  $ (the image of $i$ under this map
...
\]
(Note that $\operatorname*{Comp}A$ depends not only on the set $A$ itself, but
also on $n$. We hope that $n$ will always be clear from the context when we
use this notation.)
...
\]
(where the sum is over all permutations $L$).
\end{theorem}
...
\ref{prop.K.ideal} for $\tvi$-ideals, $\bel$-ideals, $\left.  \prec\right.
$-ideals and $\left.  \succeq\right.  $-ideals?
\end{question}
...
$\left.  \prec\right.  $-ideals is to study dendriform shuffles (i.e., left
shuffles and right shuffles). There is a well-known notion of dendriform
shuffles of words. Specialized to permutations, it can be defined in a simple
way as follows:
...
has a minimum element. Assume that each $\min P>_{\mathbb{Z}}\min Q$. Consider
the disjoint union $P\sqcup Q$ of $P$ and $Q$.
...
\textit{dendriform-shuffle-compatible} if for any two disjoint nonempty
permutations $\pi$ and $\sigma$ having the property that%
\begin{equation}
\text{the first entry of }\pi\text{ is greater than the first entry of }%
\sigma\text{.} \label{eq.def.dendri.dsc.ass}%
\end{equation}
the two multisets%
\[
...
dendriform-shuffle-compatible} if for any two disjoint nonempty permutations
...
\begin{itemize}
\item \textit{Statement A:} The statistic $\operatorname*{st}$ is dendriform-shuffle-compatible.

\item \textit{Statement B:} The statistic $\operatorname*{st}$ is weakly dendriform-shuffle-compatible.
...
Statement B holds. Thus, the statistic $\operatorname*{st}$ is weakly dendriform-shuffle-compatible.
...
further composition. If $L$ is empty, then
(\ref{pf.thm.dendri.K.ideal.two-incs}) holds for easy reasons (after all,
$F_{L}=1$); thus, we WLOG assume that $L$ is nonempty. For a similar reason,
we WLOG assume that $J\neq K$; hence, both $J$ and $K$ are nonempty (since the
only permutation $\operatorname*{st}$-equivalent to the empty permutation is itself).
...
The statistic $\operatorname*{st}$ is weakly dendriform-shuffle-compatible.
Thus, the multiset $\left\{  \operatorname*{st}\left(  \tau\right)
\ \mid\ \tau\in S_{\prec}\left(  \pi,\sigma\right)  \right\}  $ (where $\pi$
is a nonempty permutation disjoint from $\sigma$ and having the property that
each entry of $\pi$ is greater than each entry of $\sigma$) depends only on
$\operatorname*{st}\left(  \pi\right)  $ and $\left\vert \pi\right\vert $ (by
the definition of \textquotedblleft weakly
dendriform-shuffle-compatible\textquotedblright). Therefore, the multisets
$\left\{  \operatorname*{st}\left(  \tau\right)  \ \mid\ \tau\in S_{\prec
}\left(  \varphi,\sigma\right)  \right\}  $ and $\left\{  \operatorname*{st}%
\left(  \tau\right)  \ \mid\ \tau\in S_{\prec}\left(  \psi,\sigma\right)
\right\}  $ are equal (since $\left\vert \varphi\right\vert =\left\vert
\psi\right\vert $ and $\operatorname*{st}\left(  \varphi\right)
...
\ \ \ \ \ \ \ \ \ \ F_{\operatorname*{Comp}\psi}\left.  \prec\right.
F_{\operatorname*{Comp}\sigma}=\sum_{\chi\in S_{\prec}\left(  \psi
,\sigma\right)  }F_{\operatorname*{Comp}\chi}.
\]
Hence,%
\begin{align*}
F_{\operatorname*{Comp}\varphi}\left.  \prec\right.  F_{\operatorname*{Comp}%
...
$\left.  \prec\right.  $-ideal of $\operatorname*{QSym}$. Similarly,
$\mathcal{K}_{\operatorname*{st}}$ is a $\left.  \succeq\right.  $-ideal of
$\operatorname*{QSym}$. Combining these facts, we see that Statement C holds.
This proves the implication B$\Longrightarrow$C.
...
$\left.  \prec\right.  $-ideal and a $\left.  \succeq\right.  $-ideal of
$\operatorname*{QSym}$. Thus, $\mathcal{K}_{\operatorname*{st}}$ is an ideal
of $\operatorname*{QSym}$ (by Proposition \ref{prop.ideal-crit3}, applied to
$M=\mathcal{K}_{\operatorname*{st}}$).
...
\preceq\right.  F_{\operatorname*{Comp}\sigma}\equiv F_{\operatorname*{Comp}%
\left(  \pi^{\prime}\right)  }\left.  \preceq\right.  F_{\operatorname*{Comp}%
...
From $F_{\operatorname*{Comp}\pi}\left.  \preceq\right.
F_{\operatorname*{Comp}\sigma}\equiv F_{\operatorname*{Comp}\left(
\pi^{\prime}\right)  }\left.  \preceq\right.  F_{\operatorname*{Comp}\left(
\sigma^{\prime}\right)  }\operatorname{mod}\mathcal{K}_{\operatorname*{st}}$,
we obtain%
\begin{equation}
...
$\left\vert \pi\right\vert $ and $\left\vert \sigma\right\vert $. Similarly,
the same holds for the multiset $\left\{  \operatorname*{st}\left(
\tau\right)  \ \mid\ \tau\in S_{\succ}\left(  \pi,\sigma\right)  \right\}  $.
Hence, the statistic $\operatorname*{st}$ is dendriform-shuffle-compatible. In
other words, Statement A holds. This proves the implication C$\Longrightarrow$A.
...
Thus, in particular, any weakly dendriform-shuffle-compatible descent
statistic must be shuffle-compatible (because Theorem \ref{thm.dendri.K.ideal}
shows that its kernel is an $\left.  \prec\right.  $-ideal and a $\left.
\succeq\right.  $-ideal of $\operatorname*{QSym}$, whence this kernel must be
an ideal of $\operatorname*{QSym}$, whence we conclude that the statistic is
shuffle-compatible because of Proposition \ref{prop.K.ideal}). Note that this
is only true for descent statistics! As far as arbitrary permutation
statistics are concerned, this is false; for example, the number of inversions
is weakly dendriform-shuffle-compatible but not shuffle-compatible.

On the other hand, every dendriform-shuffle-compatible statistic is
shuffle-compatible (whether or not it is a descent statistic). This follows
from a careful look at the definition of \textquotedblleft
dendriform-shuffle-compatible\textquotedblright\ and the observation that
$S_{\prec}\left(  \pi,\sigma\right)  =S_{\succ}\left(  \sigma,\pi\right)  $
and $S_{\succ}\left(  \pi,\sigma\right)  =S_{\prec}\left(  \sigma,\pi\right)
$ for any two disjoint permutations $\pi$ and $\sigma$.
...
Another part of Question \ref{quest.dendri-shuf-comp} is answered by the
following (easily verified) proposition:
...
\[
\sum_{\left(  b\right)  }\left(  S\left(  b_{\left[  1\right]  }\right)  \bel
...
$\operatorname*{QSym}$. Then,%
\[
\sum_{\left(  b\right)  }\left(  S\left(  b_{\left[  1\right]  }\right)  \tvi
...
\begin{align*}
\sum_{\left(  b\right)  }\left(  S\left(  b_{\left[  1\right]  }\right)  \bel
a\right)  b_{\left[  2\right]  } &  =\underbrace{\sum_{\left(  b\right)
...
the following analogue of Theorem~\ref{thm.4.3}:
