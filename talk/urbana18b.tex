...
\item The \defn{major index $\maj \pi$} of a permutation $\pi$
      ...
      \textbf{Example:} $\maj \tup{3, 1, 5, 2, 4} = 4$.
...
\item In other words, $\st$ is shuffle-compatible if and only if
      the distribution of
      $\st$ on the set $S \tup{ \pi , \sigma }$
      stays unchanged if $\pi$ and $\sigma$ are replaced by two
...
\item \textbf{Theorem.} A descent statistic $\st$ is
...
\item \textbf{Well-known fact:} The number of lacunar subsets of
      $\ive{n}$ is the Fibonacci number $f_{n+2}$.
...
\only<1>{
\item Recall the \textbf{Theorem:} A descent statistic $\st$ is
      shuffle-compatible if and only if $\calK_{\st}$ is an
      ideal of $\QSym$.
}
\pause
...
= \sum_{i_1 \leq i_2 \leq \cdots \leq i_p} x_{i_1}^{\alpha_p} x_{i_2}^{\alpha_{p-1}} \cdots x_{i_p}^{\alpha_1} .
\]
}
