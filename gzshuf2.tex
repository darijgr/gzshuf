...
%TCIDATA{LastRevised=Thursday, October 05, 2017 18:22:35}
...
and Yan Zhuang (\cite{part1})}. We shall liberally use the notations
introduced in \cite{part1} as well as its numbering of results and references.
...
\[
\left[  \pi\right]  _{\operatorname*{Epk}}\mapsto K_{n,\operatorname*{Epk}\pi
}^{\mathcal{Z}}%
\]
is a $\mathbb{Q}$-algebra isomorphism from $\mathcal{A}_{\operatorname*{Epk}}$
...
We can rewrite Theorem \ref{thm.dendri.4.1} as follows:

\begin{corollary}
\label{cor.dendri.4.1}For any permutation $\varphi$, let $\operatorname*{Comp}%
\varphi$ denote the descent composition of $\varphi$. Let $\pi$ and $\sigma$
be two disjoint nonempty permutations. Assume that%
\[
\text{the first entry of }\pi\text{ is greater than the first entry of }%
\sigma\text{.}%
\]
...
easily follows from Theorem \ref{thm.dendri.K.ideal}:
