...
%TCIDATA{LastRevised=Tuesday, February 27, 2018 23:24:31}
...
and Yan Zhuang (\cite{part1})} (but can be read independently from the latter).
...
Whenever $\prec$ denotes some strict order, the corresponding
weak order will be denoted by $\preccurlyeq$.
(Thus, $a \preccurlyeq b$ means ``$a \prec b$ or $a = b$''.)
\end{convention}
...
be a labeled poset whose ground poset $P\sqcup Q$ is the disjoint union of
...
We shall first give a proof of Theorem \ref{thm.Epk.sh-co} using
quasisymmetric functions, and then give an independent proof of Theorem
\ref{thm.Epk.sh-co} \textbf{(a)} that proceeds as elementary as possible (but
is really just the first proof, with most of the algebra stripped away).
...
\]
is a $\mathbb{Q}$-algebra isomorphism from $\mathcal{A}_{\operatorname*{Epk}}$
to $\Pi_{\mathcal{Z}}$. Hence, Theorem \ref{thm.Epk.sh-co} \textbf{(b)} is proven.
\end{proof}

\begin{proof}
[Alternative proof of Theorem \ref{thm.Epk.sh-co} \textbf{(a)}.]We must prove
that $\operatorname*{Epk}$ is shuffle-compatible. In other words, we must
prove that for any two disjoint permutations $\pi$ and $\sigma$, the multiset
$\left\{  \operatorname*{Epk}\left(  \tau\right)  \ \mid\ \tau\in S\left(
\pi,\sigma\right)  \right\}  $ depends only on $\operatorname*{Epk}\left(
\pi\right)  $, $\operatorname*{Epk}\left(  \sigma\right)  $, $\left\vert
\pi\right\vert $ and $\left\vert \sigma\right\vert $. In other words, we must
prove that if $\pi$ and $\sigma$ are two disjoint permutations, and if
$\pi^{\prime}$ and $\sigma^{\prime}$ are two disjoint permutations satisfying
...
\sigma\right\vert =\left\vert \sigma^{\prime}\right\vert $, then the multiset
$\left\{  \operatorname*{Epk}\left(  \tau\right)  \ \mid\ \tau\in S\left(
\pi,\sigma\right)  \right\}  $ equals the multiset $\left\{
\operatorname*{Epk}\left(  \tau\right)  \ \mid\ \tau\in S\left(  \pi^{\prime
},\sigma^{\prime}\right)  \right\}  $.
...
an $m$-permutation. Hence, Corollary \ref{cor.KnEpk-prodrule} yields
\begin{align*}
...
\text{(by Proposition \ref{prop.when-Epk}, applied to }n+m\text{ instead of
}n\text{)}%
\end{array}
\right) \\
...
\ \mid\ \tau\in S\left(  \pi,\sigma\right)  \right\}  $ equals the multiset
$\left\{  \operatorname*{Epk}\left(  \tau\right)  \ \mid\ \tau\in S\left(
\pi^{\prime},\sigma^{\prime}\right)  \right\}  $ (because both of these
multisets consist of lacunar nonempty subsets $\Lambda$ of $\left[
n+m\right]  $, and the previous sentence shows that these subsets appear with
equal multiplicities in them). This completes our proof of Theorem
\ref{thm.Epk.sh-co} \textbf{(a)}.
\end{proof}
...
$\left\{  \operatorname*{st}\left(  \tau\right)  \ \mid\ \tau\in S\left(
\pi,\sigma\right)  \right\}  =\left\{  \operatorname*{st}\left(  \tau\right)
\ \mid\ \tau\in S\left(  \pi^{\prime},\sigma^{\prime}\right)  \right\}  $ as
multisets. This will show that the multiset \newline$\left\{
\operatorname*{st}\left(  \tau\right)  \ \mid\ \tau\in S\left(  \pi
,\sigma\right)  \right\}  $ depends only on $\operatorname*{st}\left(
...
\ \mid\ \chi\in S\left(  \pi,\sigma\right)  \right\}  =\left\{
\operatorname*{st}\left(  \chi\right)  \ \mid\ \chi\in S\left(  \pi^{\prime
},\sigma^{\prime}\right)  \right\}  $ as multisets. In other words, $\left\{
\operatorname*{st}\left(  \tau\right)  \ \mid\ \tau\in S\left(  \pi
,\sigma\right)  \right\}  =\left\{  \operatorname*{st}\left(  \tau\right)
\ \mid\ \tau\in S\left(  \pi^{\prime},\sigma^{\prime}\right)  \right\}  $ as
multisets. Thus, we have proven that the multiset $\left\{  \operatorname*{st}%
\left(  \tau\right)  \ \mid\ \tau\in S\left(  \pi,\sigma\right)  \right\}  $
depends only on $\operatorname*{st}\left(  \pi\right)  $, $\operatorname*{st}%
\left(  \sigma\right)  $, $\left\vert \pi\right\vert $ and $\left\vert
\sigma\right\vert $. Hence, the statistic $\operatorname*{st}$ is
...
above second proof of Theorem \ref{thm.Epk.dend} again.
...
defined in Definition \ref{def.Des-et-al}. It was proven in \cite[\S 4.4]%
{part1} that this statistic $\operatorname*{Rpk}$ is shuffle-compatible.
Furthermore, $\operatorname*{Rpk}$ is clearly a descent statistic. Hence,
Proposition \ref{prop.K.ideal} (applied to $\operatorname*{st}%
=\operatorname*{Rpk}$) shows that $\mathcal{K}_{\operatorname*{Rpk}}$ is an
ideal of $\operatorname*{QSym}$. We now claim the following:
